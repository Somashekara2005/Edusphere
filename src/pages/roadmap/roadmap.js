...
const RoadmapPage = (props) => {
  ...
  const ResourcesSection = ({ children }) => {
    ...
    const handleSearch = async () => {
      ...
      setError(null);
      try {
        // Using a public API key for demonstration
        // In production, you should use your own API key and store it securely
        const API_KEY = 'AIzaSyDiatJTF7rTB2HG7q_WOffd5m8xoOmKUW0';
        const response = await axios.get(
          `https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=10&q=${encodeURIComponent(
            resourceParam.subtopic + " tutorial"
        ...
        if (response.data.items && response.data.items.length > 0) {
          ...
          setError('No videos found for this topic. Try a different search term.');
        }
      } catch (error) {
      ...
      <div className="flexbox resources">
        ...
        <div className="databaseFill">
          <div className="search-container">
            <h3 className="search-title">YouTube Tutorials for {resourceParam.subtopic}</h3>
            
            {isSearching ? (
              <div className="search-loading">
                <LoaderPinwheel size={24} className="spinner" />
                Searching YouTube...
              </div>
            ) : error ? (
              ...
              <div className="search-results">
                {searchResults.map((video) => (
                  ...
                  >
                    ...
                    <span className="provider">{video.snippet.channelTitle}</span>
                  </a>
                ))}
              </div>
            ) : (
